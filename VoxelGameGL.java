...
public class VoxelGameGL {
    ...
    private void createChunksProgram() throws IOException {
        ...
        int vshader = createShader("shaders/chunk" + (drawPointsWithGS ? "-points" : "") + ".vs.glsl", GL_VERTEX_SHADER, defines);
        int gshader = 0;
        if (drawPointsWithGS) {
            gshader = createShader("shaders/chunk.gs.glsl", GL_GEOMETRY_SHADER, defines);
        }
        int fshader = createShader("shaders/chunk.fs.glsl", GL_FRAGMENT_SHADER, defines);
    ...
    private void createSelectionProgram() throws IOException {
        int program = glCreateProgram();
        int vshader = createShader("shaders/selection.vs.glsl", GL_VERTEX_SHADER, Collections.emptyMap());
        int fshader = createShader("shaders/selection.fs.glsl", GL_FRAGMENT_SHADER, Collections.emptyMap());
        glAttachShader(program, vshader);
        glAttachShader(program, fshader);
        glLinkProgram(program);
        if (DEBUG || GLDEBUG) {
    ...
    private void createBoundingBoxesProgram() throws IOException {
        ...
        int program = glCreateProgram();
        int vshader = createShader("shaders/boundingboxes.vs.glsl", GL_VERTEX_SHADER, defines);
        int gshader = createShader("shaders/boundingboxes.gs.glsl", GL_GEOMETRY_SHADER, defines);
        int fshader = createShader("shaders/boundingboxes.fs.glsl", GL_FRAGMENT_SHADER, defines);
        glAttachShader(program, vshader);
        glAttachShader(program, gshader);
        glAttachShader(program, fshader);
        glLinkProgram(program);
    ...
    private void createCollectDrawCallsProgram() throws IOException {
        ...
        int program = glCreateProgram();
        int vshader = createShader("shaders/collectdrawcalls.vs.glsl", GL_VERTEX_SHADER, defines);
        glAttachShader(program, vshader);
        glLinkProgram(program);
        glDeleteShader(vshader);
        if (DEBUG || GLDEBUG) {
    ...
    private long meshChunkFacesAndWriteToBuffers(Chunk chunk, VoxelField vf) {
        ...
        int faceCount = new GreedyMeshing(vf.ny, vf.py, CHUNK_SIZE, CHUNK_SIZE).mesh(vf.field, new GreedyMeshing.FaceConsumer() {
            private int i;
