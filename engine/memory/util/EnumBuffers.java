...
public final class EnumBuffers {
    ...
    public static <E extends HasValue<String>> PointerBuffer toString(MemoryStack stack, Collection<E> values) {
        PointerBuffer buffer = stack.mallocPointer(values.size());

        int i = 0;
        for (E value : values) {
            buffer.put(i, stack.ASCII(value.getValue()));
            i++;
        }
    ...
    public static <E extends HasValue<Integer>> IntBuffer toInt(MemoryStack stack, Collection<E> values) {
        IntBuffer buffer = stack.mallocInt(values.size());

        int i = 0;
        for (E value : values) {
            buffer.put(i, value.getValue());
            i++;
        }
    ...
    public static <E extends HasValue<Long>> LongBuffer toLong(MemoryStack stack, Collection<E> values) {
    ...
    public static <E extends HasValue<Float>> FloatBuffer toFloat(MemoryStack stack, Collection<E> values) {
